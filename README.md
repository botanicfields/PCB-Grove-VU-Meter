...
## 安全上の注意事項
- M5StackのM-BUS直接での5V電源供給は推奨されておらず自己責任とされています。 M5Stack側の5Vと本基板の5VがGPIO上で競合すると、僅かな電圧差から大電流が流れ、素子の発熱による火傷や、M5Stackの故障の原因になります。
...
- 本基板から5Vを供給(ON)する場合は、USB-Cケーブルを取り外し、M5Stackを電源OFF※としてから、DCプラグを差し込んでください。バッテリ動作中のM5Stackは、電源/リセットボタンを2回押すことで電源をOFFにできます。
- Groveへの5V出力は、必要な場合以外はジャンパピンJP3で切り離し(OFF)てください。特に電源が3.3Vの装置を接続すると、焼損などの致命的な事故の原因となります。
